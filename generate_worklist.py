...
parser.add_argument('-add', '--addsd', metavar = 'ADD_SOURCE_DEST', type = str, nargs = '+', choices = [x + y for x in [chr(l) for l in range(65,73)] for y in [chr(l) for l in range(65,73)]] + [str(x) for x in range(12)], help = 'first argument is the location where to change the pid (0=source, 1=destination, 2=both); second argument is to add to the source or destination position id a certain number of positions to shift it; the third and fourth argment are the source & destination column numbers to start at; fifth argument is concentration of both well id prefix for alphanumeric to change the original well id to, should be a letter such as AH')
